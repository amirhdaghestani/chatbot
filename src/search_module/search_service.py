...
def run_chat(query_string: str, query_phrase: str, keywords: list, restriction, 
             search_level=gs.QUICK_SEARCH, num_urls: int=2):
  #tracemalloc.start()
  response_text = ''
  ...
  try:
    ...
    try:
      ...
      google_text, urls_all, index, urls_used, tried_index, urls_tried = \
        gs.search_google(query_string, search_level, query_phrase, keywords, [], restriction=restriction, num_urls=num_urls)
