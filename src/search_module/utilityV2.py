...
def split_interaction(interaction):
    ...
    else: print("can't parse", interaction)
    return '',''
...
keyword_prompt = 'Perform two tasks on the following text in Persian. Do not use English in any task. First, rewrite the <text> as an effective google search phrase. Second, analyze text and list keywords and named-entities found. Return the result as: Phrase: "<google search phrase>"\nKeywords: <list of keywords>\nNamed-Entities: <list of Named-Entities>'
  ...
  for action_index in action_indecies:
      ...
      if query_start > 0:
        ...
        if query_end > 0:
          ...
          action = text[action_index.start():action_index.span()[1]+action_index.start()+query_start+query_end+2]
      if agent is None or query is None:
        print("can't parse action, skipping", text[action_index.start():action_index.start()+48])
        continue
...
if __name__ == '__main__' :
  get_search_phrase_and_keywords("Would I like the video game Forspoken, given that I like Final Fantasy VII?", [])
