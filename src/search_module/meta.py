from . import utilityV2 as ut
from . import google_search_concurrent as gs
import re
import time
...
def parse_decomposition(text):
    ...
    subquery_indecies = re.finditer('<Subquery', text) # Action: Ask {Google, User} "query"
    subqueries = []
...
def query_keywords(query):
    ...
    gpt_key_message = [{'role':'user', 'content':'Extract keywords and named-entities from the following text.'}, 
                       {'role':'user', 'content':query}
                       ]
    #for item in gpt_key_message:
    #    print(item)
    gpt_parse = ut.ask_gpt_with_retries('gpt-3.5-turbo', gpt_key_message, tokens=25, temp=0, timeout=5, tries=2)
    ...
    if ne_start > 0:
        nes = gpt_parse[ne_start+len('Named-entities')+1:].split(',') # assume string ends with colon or space:].split(',')
...
def substitute(Q1, A1, Q2, debug=False):
    gpt_sub_message = [{'role':'user', 'content':"replace '"+Q1+"' with '"+A1+"' in '"+Q2+"'"}]
    if debug:
        print('\n\n**************')
        for item in gpt_sub_message:
            print(item)
    google_tldr = ut.ask_gpt_with_retries('gpt-3.5-turbo', gpt_sub_message, tokens=25, temp=.1, timeout=5, tries=2)
    print('\n\n**************')
...
def meta(query, chat_history, debug=False):
    print("***** entering meta")
    turn = ut.turn(role=ut.ASSISTANT, source = ut.ASSISTANT, message='Action: search "'+query+'"')
...
<Subquery 2>"""
    ...
    response_text = ut.ask_gpt_with_retries('gpt-3.5-turbo', gpt_message, tokens=75, temp=.1, timeout=5, tries=2)
    if debug:
    ...
    for n, subquery in enumerate(subqueries):
        ...
        gpt_initial_message = [{'role':'user', 'content':subquery+" If fact is unavailable, respond: 'Unknown'"}]

        #for turn in meta_chat_history:
        #    gpt_initial_message.append({"role":"user","content":turn.tldr})
                                        
        initial_gpt_answer = ut.ask_gpt_with_retries('gpt-3.5-turbo', gpt_initial_message, tokens=25, temp=0.0, timeout=5, tries=2)
        if debug:
            print(f'***** google extract\n {initial_gpt_answer}\n')
        if ('unknown' not in initial_gpt_answer.lower()
            and 'cannot provide' not in initial_gpt_answer
            and 'do not have access' not in initial_gpt_answer):
            meta_chat_history.append(ut.turn(role='assistant', message=subquery, source=ut.ASSISTANT, tldr=subquery, keywords=keyword_set))
            meta_chat_history.append(ut.turn(role='assistant', message="<note>\n"+initial_gpt_answer+'\n<note>', source=ut.GOOGLE, tldr=initial_gpt_answer, keywords=keyword_set))
        ...
        google_text, urls_all, index, urls_used, tried_index, urls_tried = \
            gs.search_google(subquery, gs.QUICK_SEARCH, '', ut.INFORMATION_QUERY, keyword_set, meta_chat_history)
        if len(google_text) > 0:
            #digest google response into an answer for this subquery
            if debug:
                print(f"***** search result\n{google_text}\n")
            gpt_tldr_message = [{'role':'user', 'content':'Summarize the set of <note> provided. Including only the direct answer to <Query>. Do not include any qualifiers or modifiers from the <Query> such as "where x was born".'},
                                {'role':'user', 'content':google_text},
                                {'role':'user', 'content':'<Query>\n'+subquery}]
            #for turn in meta_chat_history:
            #   gpt_tldr_message.append({"role":"user","content":turn.tldr})
                                        
            google_tldr = ut.ask_gpt_with_retries('gpt-3.5-turbo', gpt_tldr_message, tokens=150, temp=.1, timeout=5, tries=2)
            #print('\n\n**************')
            #for item in gpt_tldr_message:
            #    print(item)
            print(f"***** Answer to {subquery}: {google_tldr}\n")
            meta_chat_history.append(ut.turn(role='assistant', message=subquery, source=ut.ASSISTANT, tldr=subquery, keywords=keyword_set))
            meta_chat_history.append(ut.turn(role='assistant', message="Observation: "+google_tldr, source=ut.GOOGLE, tldr=google_tldr, keywords=keyword_set))
...
if __name__ == '__main__' :
    ...
    meta("What are the latest reviewer opinions on Tesla Full Self Driving Beta version 11.3.4?", [], debug=True)
